...
app.get(/^\/icons\/(?:android-chrome-\d+x\d+|apple-touch-icon)\.png$/, (req, res) => res.type('image/png').sendFile(path.join(__dirname, req.path)));
...
app.post('/api/media', async (req, res) => {
  try {
    ...
    if (directType === 'pdf') {
      throw new Error('PDF files are not supported. Enter a direct image, video, or audio URL.');
    }
